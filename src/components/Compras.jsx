...
export default function PurchaseCards(props) {
  ...
  const submitHandler = async (e) => {
    ...
    const ratings = await axios.get(`${VITE_LOCAL_HOST}/rating/user/${user.id}`);
      ...
      <Stack bg={""} w={"85%"} ml={"2vh"}>
        <CardBody>
          <Flex direction="row" align="center" justify="space-between">
            ...
            {/* <Box> */}
              <Popover bg={'green'} isOpen={isOpen} onClose={() => setIsOpen(false)}>
                <PopoverTrigger>
                    {/* Contenido visible del PopoverTrigger */}
                    <Button onClick={() => setIsOpen(true)} variant="solid" color="#ffa200" bg={"#1b1b1b"}>
                      Puntuar
                    </Button>
                </PopoverTrigger>
